...
function getExpress(pageNum){
	$.get("ce/list?page="+pageNum,function(data){
		//alert(data.total);
		$("#expressList").empty();
...
function getType(){
	$.get("ce/type",function(data){

		$("#typelist").empty();
		var sum=0;
		for(var i=0;i<data.length;i++){
			$("#typelist").append(
						"<span><a href='javascript:void(0)'  id='ce/findType?otype="+data[i].otype+"'>"+data[i].otype
...
function getdidbyuuid(){
	$.post("ce/getdidbyuuid",{"uuid":param},function(data){
		if(data!=null){
			$("#hidddid").val(data);
