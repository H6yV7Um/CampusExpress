 ...
 */
@WebFilter("/*")
public class AdminCheckFilter extends AbstractFilter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (request.getSession().getAttribute("loginAdmin") == null) {//当没有登录时,为空.当其他用户登录时,这里会被清空
			if (requri.endsWith("page/manage.jsp")  ) {
				// 2.满足过滤的过滤条件就终止继续请求,
				...
				LogManager.getLogger().debug("你好,管理员!请先登录再操作");
				request.getRequestDispatcher("/page/loginAdmin.jsp").forward(request, response);
